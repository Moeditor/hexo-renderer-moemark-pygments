...
let render = function (data, options, cb) {
	...
	let highlightConfig = this.config.highlight || {};
	let highlightOption = {
      autoDetect: highlightConfig.auto_detect,
      gutter: highlightConfig.line_number,
      tab: highlightConfig.tab_replace
    };
    ...
    MoeMark.setOptions({
        ...
        highlight: (code, lang) => {
			...
			let options = Object.assign({ lang: lang }, highlightOption);
            let res = util.highlight(stripIndent(code), options);
